...
RUN cd /www && \\
    ln -sv ${volume}/swagger.json swagger.json && \\
    chmod +rX ${volume} && \\
    chmod +rX -R ${volume}/swagger.json
...
# patch swagger file
RUN perl -pi -w -e 's!http://petstore.swagger.io/v2/swagger.json!http://localhost:${port}/swagger.json!g;' /www/index.html
...
if [ "$(uname)" = "Darwin" ]; then
  docker-machine ssh dev -f -N -L ${port}:localhost:${port}
fi
